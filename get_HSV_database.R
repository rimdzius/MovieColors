...
get_HSV_database <- function(temp_pic_dir, movie_position, total_movies) {
      ...
      progress <- progress_bar$new(format = paste0("   evaluating movie ", movie_position, "/", total_movies, " [:bar] :percent eta: :eta"),
                                   total = number_of_frames)
      progress$tick(0)
      
      for (frame in 1:number_of_frames) {
            # iterate over each image
            image_file <- paste0(temp_pic_dir,"/","image_",sprintf("%06d", frame),".png")
